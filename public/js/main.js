...
window.getMemberName = function (el) {
    ...
    if (el.parents('.input-group').length) {
        el = el.parents('.input-group').first();
    }
    if (code.length >= 6) {
        $.get({
...
//Fetch Package Name
window.getPackageName = function (el) {
    let code = el.val();
    if (el.parents('.input-group').length) {
        el = el.parents('.input-group').first();
    }
    if (code.length >= 6) {
        ...
        let panel = currentRoute.substr(0, currentRoute.indexOf('.'));

        $.get({
            url: route('packageName', code),
            success: function (res) {
                el.siblings('.packageName').remove();
                if (res.pin != null) {
                    el.after(
                        '<span class="help-block packageName text-primary font-weight-bold">' + res.pin.package.name + '</span>'
                    );
                } else {
                    el.after(
                        '<span class="help-block packageName text-danger font-weight-bold">Package not found</span>'
                    );
                }
            },
            error: function (res) {
                ...
                if (res.status === 404) {
                    ...
                    el.after(
                        '<span class="help-block packageName text-danger font-weight-bold">Something went wrong, please try again.</span>'
                    );
                }
            }
        });
